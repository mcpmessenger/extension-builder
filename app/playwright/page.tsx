...
import { Camera, Play, Download, Trash2, Eye, Sparkles, ArrowRight, Trash } from "lucide-react";
...
export default function PlaywrightPage() {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 mx-auto max-w-4xl p-6 space-y-6">
        <h1 className="text-2xl font-semibold text-white drop-shadow-lg">Capture Clickable Link Screenshots</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] items-center">
          ...
          />
          <Input
            className="sm:ml-2 bg-white/90 backdrop-blur-sm border-white/30"
            placeholder="output folder"
            value={folder}
            onChange={(e) => setFolder(e.target.value)}
          />
          <Button className="sm:ml-2 bg-blue-600 hover:bg-blue-700 text-white shadow-lg" onClick={runCapture} disabled={isPending}>
        ...
        {images.length > 0 && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {images.map((src) => (
                <div key={src} className="rounded border border-white/20 overflow-hidden relative group bg-black/20 backdrop-blur-sm">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={src} alt={src} className="w-full h-auto cursor-zoom-in" onClick={() => setOpenSrc(src)} />
                  <div className="px-2 py-1 text-xs truncate text-white drop-shadow-sm bg-black/50">{src.split("/").pop()}</div>
                  <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <a
                      className="text-xs bg-blue-600/90 hover:bg-blue-700 text-white px-2 py-1 rounded shadow-lg"
                      href={src}
                      download
                    >
                      Download
                    ...
                    <button
                      className="text-xs bg-red-600/90 hover:bg-red-700 text-white px-2 py-1 rounded shadow-lg"
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-6">
              ...
              >
                ...
                {busy === "delete-all" ? "Deleting All..." : "Delete All Screenshots"}
              </Button>
